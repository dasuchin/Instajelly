...
class UserModel extends Model {
	...
	function __construct($user = '') {
		...
		if(is_string($user)) {
			...
			$userInfo = $this->findUserByUsername($user);
	...
	function getRecentUsers($type = 'all', $number = 1) {
	...
	function saveUser($data) {
		...
		//check to see if user is admin or regular user
		if($data['type'] == 'admin') {
			...
			if(!empty($data['username'])) {
				...
				$usernameUsedBy = $this->usernameUsed($data['username']);
				$data['id'] = !empty($data['id'])?intval($data['id']):0;
				
				if($usernameUsedBy != false && $usernameUsedBy != $data['id']) {
					
					$this->errorMsg = 'Username already in use. '.$usernameUsedBy.' - '.$data['id'];
					//return false;
	...
	function findUserByUsername($username) {
		...
		$sql = "SELECT * FROM `users` WHERE `username` = '".$this->db->makeSafe($username)."'";
